...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Aura Beauty Studio — Centro Estetico di Lusso" },
      { name: "description", content: "Trattamenti viso, corpo, epilazione laser e anti-age personalizzati. Valorizza la tua bellezza naturale con specialisti certificati." },
      { property: "og:title", content: "Aura Beauty Studio" },
      { property: "og:description", content: "Trattamenti professionali personalizzati per il tuo benessere." },
...
const reviews = [
  {
    ...
    text: "Ogni visita ad Aura è un piccolo rituale. Pelle radiosa, ambiente meraviglioso e personale sempre attento. Lo consiglio davvero a tutte.",
  },
  {
    name: "Sara B.",
    role: "Trattamento viso",
    text: "Finalmente un centro che ascolta. Il protocollo personalizzato ha trasformato la mia pelle in poche settimane. Risultati visibili e duraturi.",
  },
  {
    name: "Martina R.",
    role: "Epilazione laser",
    text: "Professionalità e cura del dettaglio impeccabili. Mi sono sentita coccolata dall'inizio alla fine. Aura è la mia nuova oasi.",
  },
];
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        ...
        <div className="mt-16 grid md:grid-cols-3 gap-6">
          {reviews.map((r) => (
            <figure
              key={r.name}
              className="bg-card rounded-3xl border border-border/60 p-8 lg:p-10 hover:shadow-soft transition-shadow"
            >
              <div className="flex items-center gap-1 text-gold">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className="h-4 w-4 fill-current" />
        ...
        <div className="relative overflow-hidden rounded-[2rem] lg:rounded-[3rem] bg-gradient-primary text-primary-foreground px-8 py-16 sm:px-14 sm:py-20 lg:px-20 lg:py-28 text-center">
          ...
          <div className="relative mt-10 flex flex-wrap items-center justify-center gap-4">
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full bg-background text-foreground px-7 py-4 text-sm font-medium hover:bg-card transition-colors"
            >
              <Phone className="h-4 w-4" />
              Chiama ora
            </a>
            <a
              href="mailto:info@aurabeauty.it"
              className="inline-flex items-center gap-2 rounded-full bg-gradient-gold text-gold-foreground px-7 py-4 text-sm font-medium hover:opacity-90 transition-opacity"
            >
              Prenota online
              <ArrowRight className="h-4 w-4" />
            </a>
